...
export class Upcomingtours {
  ...
  getTrekById(id: string | number): Observable<any> {
    const trekId = String(id ?? "").trim();
    if (!trekId) {
      return throwError(() => new Error('Invalid trek ID'));
    }

    return this.http.get<any>(`${this.apiUrl}/${trekId}`).pipe(
      map((res: any) => {
        try {
          if (res && res.data) {
      ...
      map(response => {
        try {
          ...
          if (decrypted?.success && Array.isArray(decrypted.treks)) {
            return decrypted.treks
              .map((trek: any) => this.formatTrekForList(trek))
              .filter((trek: TrekListItem | null): trek is TrekListItem => !!trek);
          }
  ...
  private formatTrekForList(trek: any): TrekListItem | null {
   ...
   */
  private formatTrekWithBatch(trek: any, batch: any): TrekListItem | null {
