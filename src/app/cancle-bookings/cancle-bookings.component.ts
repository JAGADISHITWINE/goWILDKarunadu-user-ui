...
export class CancleBookingsComponent implements OnInit {
  ...
  refundChannels: DropdownOption[] = [];
  selectedRefundChannel = 'Online Gateway Reversal (Razorpay)';
  refundedAmount: any;
  ...
  ngOnInit() {
    ...
    this.loadRefundChannels();
  }

  private loadCancellationReasons() {
    const fallback: DropdownOption[] = [
      { value: 'Customer Cancellation', label: 'Change of plans / Personal reasons' },
      { value: 'Medical Emergency', label: 'Medical Emergency / Fitness issue' },
      { value: 'Weather Concerns', label: 'Weather Concerns / Rain alert' },
      { value: 'Work Commitments', label: 'Work & Official commitments' },
      { value: 'Found Alternative Trek', label: 'Found Alternative Trek / Date conflict' },
  ...
  private loadRefundChannels() {
    const fallback: DropdownOption[] = [
      { value: 'Online Gateway Reversal (Razorpay)', label: 'Original Payment Method (Direct Gateway Reversal)' },
      { value: 'UPI Instant Payout', label: 'UPI Instant Payout (GPay / PhonePe / Paytm)' },
      { value: '100% Credit Shell Voucher', label: '100% Trek Credit Shell (Valid 12 Months)' },
      { value: 'Direct Bank Transfer (NEFT)', label: 'Direct Bank Transfer (NEFT / IMPS)' },
    ];

    this.dropdownService.getOptions('refund-channels', fallback).subscribe((options) => {
      this.refundChannels = options;
    });
  }
  ...
  submitCancellation() {
    ...
    const otherData = {
      ...
      refundChannel: this.selectedRefundChannel,
