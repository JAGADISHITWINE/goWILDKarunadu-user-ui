...
import { ReferralService, ReferralSummary } from '../core/referral.service';
...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  isLoading = false;
  errorMessage = '';
  readonly initialVisibleCount = 8;
  ...
  referralError = '';
  referralRequiresAuth = false;
  referralShareLink = '';
  referralCopyFeedback = '';
  isReferralDrawerOpen = false;
  private referralFeedbackTimeout: any = null;
  ...
  private resolveImageUrl(imagePath: string | null | undefined): string {
  ...
  get referralNextRewardLabel(): string {
  ...
  refreshReferralSummary(): void {
    this.loadReferralSummary();
  }

  async copyReferralCode(): Promise<void> {
    if (!this.referralSummary?.referralCode) return;
    try {
      await this.copyToClipboard(this.referralSummary.referralCode);
      this.setReferralFeedback('Referral code copied');
    } catch {
      this.setReferralFeedback('Copy failed. Long press to copy manually.');
    }
  }

  async copyReferralLink(): Promise<void> {
    if (!this.referralSummary?.referralCode) return;
    const link = this.referralShareLink || this.buildShareLink(this.referralSummary.referralCode);
    if (!link) return;
    try {
      await this.copyToClipboard(link);
      this.setReferralFeedback('Invite link copied');
    } catch {
      this.setReferralFeedback('Unable to copy link right now.');
    }
  }
  ...
  async shareReferral(): Promise<void> {
    if (!this.referralSummary?.referralCode) return;
    const link = this.referralShareLink || this.buildShareLink(this.referralSummary.referralCode);
    const shareText = `Join me on a Go Wild Karunadu trek and use my code ${this.referralSummary.referralCode} for instant savings.`;
    if (link && typeof navigator !== 'undefined' && navigator.share) {
      try {
        ...
        this.setReferralFeedback('Invite shared');
        return;
      } catch (err: any) {
  ...
  openReferralLogin(): void {
    this.authModal.openLogin()
      .then(() => this.loadReferralSummary())
      .catch(() => void 0);
  }
  ...
  private setReferralFeedback(message: string): void {
