...
export class DropdownService {
  ...
  getAllDropdowns(): Observable<Record<string, DropdownOption[]>> {
    return this.http.get<{ success: boolean; data: any }>(`${this.API}/meta/dropdowns`).pipe(
      map((res) => {
        const decrypted = this.tryDecrypt(res?.data) || res?.data || {};
        return decrypted;
      }),
      catchError(() => of({}))
    );
  ...
  getGearRentals(): Observable<any[]> {
    return this.http.get<{ success: boolean; data: any[] }>(`${this.API}/meta/gear-rentals`).pipe(
      map((res) => res?.data || []),
      catchError(() => of([]))
    );
  }

  getTrailAdvisories(): Observable<any[]> {
    return this.http.get<{ success: boolean; data: any[] }>(`${this.API}/meta/trail-advisories`).pipe(
      map((res) => res?.data || []),
      catchError(() => of([]))
    );
  }
